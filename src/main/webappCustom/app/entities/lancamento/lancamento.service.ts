...
export class LancamentoService {
    private resourceUrl = SERVER_API_URL + 'api/custom/lancamentos';
    ...
    queryFilter(descricao): Observable<EntityArrayResponseType> {
        const options = createRequestOption();
        return null;
    }

    queryResumo(req?: any): Observable<EntityArrayResponseType> {
        const options = createRequestOption(req);
        console.log('=========>', options);
        return this.http
            .get<ILancamento[]>(`${this.resourceUrl}/resumo`, { params: options, observe: 'response' })
            .pipe(map((res: EntityArrayResponseType) => this.convertDateArrayFromServer(res)));
    }
